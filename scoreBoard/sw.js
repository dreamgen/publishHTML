/**
 * Service Worker for 萬用計分板 (scoreBoard) PWA
 *
...
const SW_VERSION = 'v1';
const CACHE_NAME = `scoreBoard-${SW_VERSION}`;
const SHARED_CACHE = `scoreBoard-shared-${SW_VERSION}`;
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys
          .filter((k) => k.startsWith('scoreBoard-') && !ALL_CACHES.includes(k))
