...
self.addEventListener('fetch', event => {
  ...
  // Network-First for HTML/Navigation to ensure latest content
  if (request.mode === 'navigate' || url.pathname.endsWith('.html') || url.pathname === '/' || url.pathname.endsWith('/myBadge/')) {
    event.respondWith(
      fetch(request)
        .then(res => {
          if (res?.ok) {
