/**
 * Service Worker for 骰子搖搖 (playDices) PWA
 *
...
const SW_VERSION = 'v1';
const CACHE_NAME = `playDices-${SW_VERSION}`;
const SHARED_CACHE = `playDices-shared-${SW_VERSION}`;
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(
        keys
          .filter((k) => k.startsWith('playDices-') && !ALL_CACHES.includes(k))
