...
相容性報告會依工作表列出無快取結果的公式、圖片、條件格式、外部連結、旋轉文字、漸層填色等可能改變或略過的內容。正式輸出會轉換儲存格內容、已儲存的公式結果、合併儲存格、欄寬列高、基本字型樣式、填色、框線與對齊；圖表、SmartArt、巨集、密碼保護 Excel 與部分條件格式仍不在支援範圍。原始 Excel 不會保存到草稿，只有產生的 PDF 會進入既有草稿與匯出流程。
