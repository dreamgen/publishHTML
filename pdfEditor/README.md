...
- 開啟一般與密碼保護 PDF
- 多檔合併、頁面群組、頁面排序、旋轉、刪除、範圍選取與擷取；在群組中插入單頁會直接納入該群組，插入多頁時可選擇納入目前群組或在群組後建立新群組
- 插入空白頁、圖片轉 PDF
- Excel `.xlsx` 轉 PDF，可選擇要轉換的 Sheet，並開啟為新文件或插入目前頁面後
- 文字、畫筆、螢光筆、矩形、箭頭、圖片與手寫簽名標註；游標模式可拖曳移動，並以四角控制點縮放內容
- PDF 表單填寫與扁平化
- 繁體中文＋英文 OCR（橫書），以及繁體中文直書 OCR，可辨識目前頁、所選頁或全部頁面
...
OCR 對話框提供「辨識方向」下拉選單，可選擇「橫書（繁體中文＋英文）」或「直書（繁體中文直排）」。橫書使用 `chi_tra` 與 `eng` 模型，直書使用 `chi_tra_vert` 與 `chi_tra` 模型以同時處理直排文字與橫向標題。選擇會被記住，切換模式時會於首次使用該模式時載入對應的本機模型（需數秒）。已知限制：直書模式下全形標點可能輸出為半形，模糊掃描的辨識準確度較低於橫書，重要文件請校對結果。搜尋、複製與可選取文字層對直書結果亦適用。
...
首次安裝離線版本時會快取 PDF.js、字型與 PDF 編輯核心；OCR 核心與模型（`chi_tra`、`eng`、`chi_tra_vert`；約 17MB）改為背景與首次使用時快取，讓安裝更快、更不易失敗。
