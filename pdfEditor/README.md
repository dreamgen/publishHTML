...
視窗底部狀態列另提供縮放滑桿（−／滑桿／＋／百分比），操作對象與工具列縮放一致：單頁與連續模式調整頁面縮放（50%–250%），瀏覽模式調整縮圖大小（60%–240%）；點擊百分比可重設為 100%。拖曳滑桿時即時更新數值，停頓約 0.12 秒後才重新渲染頁面以維持流暢。瀏覽模式的網格頂部（全選列右側）也有一組相同的縮放列，與底部滑桿完全同步。
