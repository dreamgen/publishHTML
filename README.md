...
- **自動離開偵測**（可在設定關閉、調整距離）：已打卡時持續比對與打卡點的距離，超過 300 m 提示是否離開、超過 1 km 自動結束打卡（可一鍵復原）；需連續 2 次在範圍外才判定，誤差 > 150 m 的定位略過
  - PWA 只有在前景時能讀取定位：App 在背景／鎖屏期間離開，會在下次開啟 App 時補判；15 分鐘內有確認在場就直接採用，否則以「最後在場時間／依距離推算的時間」記錄並標示「離開時間待確認」，覆盤與日報頁會提示修正
...
- **每月統計**：出勤天數、駐點總時數、依地點時數與占比、每日明細；匯出「統計表 CSV」與「行程明細 CSV」（含離開方式與是否待確認；UTF-8 含 BOM，Excel 開啟不亂碼）
...
> 天達堂區幹部用手機查閱服務人員資料、直接做排班問卷，並把結果寫回〈服務人員名冊〉xlsx；資料只在手機端、加密保存、不上傳

**功能：**
- **匯入名冊 xlsx 一次就記住**：讀取〈服務人員名冊〉與〈參考名冊〉，之後不用每次找檔案
- **PIN 加密**：名冊、原始 xlsx、問卷答案以 PIN 衍生的 AES-256-GCM 金鑰加密存在 IndexedDB；離開 App 逾時自動上鎖
- **搜尋與篩選**：姓名、編號、區、班別、帶隊角色、社團；依區、問卷狀態（未填／已填／待寫回）、來源篩選
- **人員資料頁**：基本資料、115 年各班出席、服務紀錄（場次、平日天數、7 天內重複、廚務／服務組／禮節／忠恕／交通）、資料備註、問卷摘要（電話可直接撥打）
- **問卷調查**：沿用〈天達堂服務人員問卷〉7 部分 31 題，自動帶入名冊的姓名／區／乾坤／年齡，邊填邊自動儲存；名冊外新人也能填，同名時提示改記在名冊人員名下
- **寫回名冊**：以原檔為底（保留格式、公式、下拉選單）填入黃底欄「工作狀態…116年意願備註」，新人加在最後一列，另新增〈問卷明細〉工作表；可下載或分享
- 匯入線上問卷網頁存下的 `.json`、匯入本 App 匯出的 xlsx 可合併多支手機的進度、加密備份／還原
- 頁面 CSP 禁止對外連線；ExcelJS 內建於 `vendor/`，完整離線支援

**安裝：** 開啟 `/staffSurvey/` 後，點選「加入主畫面」即可安裝為獨立 App（iPhone 務必安裝，避免 Safari 清除久未使用網站的資料）。詳細規則見 `staffSurvey/README.md`。
